# User-owned provisioning for BuddyBox

_Research current as of 2026-08-13. Sources are vendor documentation and official source repositories._

> Product decision, 2026-08-14: BuddyBox's beta uses centrally managed Cloudflare
...
BuddyBox keeps source repositories in the User's GitHub account and backends in the
User's Convex team while centrally hosting beta releases in BuddyBox's Cloudflare
...
| Cloudflare Worker and related resources | BuddyBox-owned during beta | Operator credential behind the deployment gateway | No User connection or OAuth gate |
...
organization, an BuddyBox-hosted release, and a project in the User's Convex team.
A Clerk application should become User-owned at claim/transfer time. BuddyBox owns
...
`<project>-buddybox-sites.buddytools.org`; collision suffixes must be stable.
...
Practical blockers are organizational policy and account binding. An app requesting repository administration will commonly require organization-owner approval, and an organization can prohibit member requests. Personal-account installation is simpler. The setup callback's `installation_id` is not proof of identity and is explicitly spoofable; BuddyBox must bind it only after validating the authenticated GitHub user and a short-lived, one-use state value ([GitHub: setup URL security warning](https://docs.github.com/en/apps/creating-github-apps/registering-a-github-app/about-the-setup-url)).

**MVP:** generate a private repository from the BuddyBox template under the selected account, retain installation/account/repository IDs, and let the Git gateway mint destination-scoped IATs for each push. Request a user access token only if user attribution or an endpoint unavailable to IATs is actually needed.
...
Workers Builds should not be on the MVP's critical path. Its current REST API requires a user-scoped API token, rejects account tokens, and requires a one-time manual Cloudflare GitHub App installation before automation ([Workers Builds API](https://developers.cloudflare.com/workers/ci-cd/builds/api-reference/)). Direct artifact upload from BuddyBox's gateway avoids that coupling.
...
2. Install the GitHub App, validate the installation against the logged-in GitHub principal, and generate the user's private repository from the BuddyBox template.
3. Authorize Convex project-scoped access and create/select a user-owned project.
4. Allocate an BuddyBox-managed hostname and deploy the approved release through the trusted Cloudflare gateway; require no User Cloudflare connection.
...
The hard launch dependencies are BuddyBox-managed hostname routing and certificate
