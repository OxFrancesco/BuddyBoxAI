...
export function repositoryMaterialization(request: AdmissionRequest): {
  command: string;
  env: { BUDDYBOX_RUN_CAPABILITY: string };
  remote: string;
} {
  const remote = `http://github.buddybox.internal/${request.repository}.git`;
  return {
    remote,
    env: { BUDDYBOX_RUN_CAPABILITY: request.capability },
    command: [
      "set -eu",
      'export GIT_CONFIG_COUNT="1"',
      'export GIT_CONFIG_KEY_0="http.extraHeader"',
      'export GIT_CONFIG_VALUE_0="Authorization: Bearer $BUDDYBOX_RUN_CAPABILITY"',
