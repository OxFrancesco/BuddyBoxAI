...
interface Principal {
  userId: string;
  projectId: string;
  action: "admission" | "run" | "cancel" | "heartbeat" | "checkpoint" | "replacement" | "preview" | "artifact";
  capability: string;
}
...
function requiredAction(method: string, suffix: string): Principal["action"] | null {
  ...
  if ((method === "POST" && suffix === "/screenshots") || (method === "GET" && suffix === "/artifacts")) return "artifact";
  return null;
}
  ...
  return {
    async fetch(request: Request): Promise<Response> {
      ...
      try {
        ...
        if (principal.projectId !== projectId || principal.action !== action) {
