...
const originalKey = process.env.BUDDYBOX_PROVIDER_CREDENTIAL_KEY;

afterEach(() => {
  if (originalKey === undefined) delete process.env.BUDDYBOX_PROVIDER_CREDENTIAL_KEY;
  else process.env.BUDDYBOX_PROVIDER_CREDENTIAL_KEY = originalKey;
});

describe("provider OAuth cryptography", () => {
  test("binds encrypted credentials to owner, provider, and purpose", async () => {
    process.env.BUDDYBOX_PROVIDER_CREDENTIAL_KEY = btoa(String.fromCharCode(...new Uint8Array(32).fill(7)));
